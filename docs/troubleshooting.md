...
npm run doctor -- --db-path 'D:\msg-mcp\data\msg.sqlite' --skip-server
...
远端检查严格验证 HTTP 成功、JSON、`service: msg-mcp`、`status: ok`、ISO 时间和 member 对象；输出中心识别的成员名。3 秒超时，拒绝跳转。URL 校验/前缀拼接与 bridge 共用，拒绝非 HTTP(S)、userinfo、query、hash。
