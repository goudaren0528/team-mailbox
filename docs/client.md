...
**单条未读免选择：** 仅当筛选范围内确实只有一条（`unread_only=true`、本轮第一页且 `cursor=0`、`messages.length=1`、`hasMore=false`）时，用户本次查看意图即授权直接先收附件、再呈现完整正文，不再弹选择。多条、全部模式、后续页或完整性不确定仍用原生 question；0 条不读不下载；取消后重新请求需重新查询。附件保存失败仍须重试／跳过／取消。
